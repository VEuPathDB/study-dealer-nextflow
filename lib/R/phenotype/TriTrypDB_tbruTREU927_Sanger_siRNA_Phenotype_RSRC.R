wrangle <- function() {
  ...
  # read in file
  genePhenotype = entity_from_file("phenotype.txt")
  ...
  genePhenotype <- genePhenotype %>%
    ...
    set_variable_metadata('pubmedId', display_order=2, display_name = "Pubmed ID", definition = "Pubmed ID") %>%
    ...
    set_variable_metadata('qualityTerm', display_order=5, display_name = "Quality Term", definition = "Quality Term") %>%
    set_variable_metadata('entityTerm', display_order=6, display_name = "Entity Term", definition = "Entity Term") %>%
    set_variable_metadata('timing', display_order=7, display_name = "Timing", definition = "Timing")  %>%
    set_variable_metadata('lifeCycleTerm', display_order=8, display_name = "Life Cycle Term", definition = "Life Cycle Term")  %>%
    set_variable_metadata('evidenceTerm', display_order=9, display_name = "Evidence Term", definition = "Evidence Term")  %>%
    set_variable_metadata('note', display_order=10, display_name = "Note", definition = "Note")
  
  crisprStudy = study("tbruTREU927_phenotype_Sanger_siRNA_RSRC", genePhenotype)
