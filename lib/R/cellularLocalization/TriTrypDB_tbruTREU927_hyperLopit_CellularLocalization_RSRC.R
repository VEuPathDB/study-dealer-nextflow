...
wrangle <- function() {
  ...
  bsf_file = "20220329_TbBSF_n3_TriTrypDB_tables.csv"
  pcf_file = "20220329_TbPCF_n3_TriTrypDB_tables.csv"
  ...
  lopitEntity <- lopitEntity %>%
    create_variable_category(
      ...
      definition = "TAGM-MAP cellular localization predictions and probabilities for T. brucei bloodstream form",
      children = bsf_vars
    ) %>%
    create_variable_category(
      category_name = "pcf_localization",
      display_name = "Procyclic Form Localization",
      definition = "TAGM-MAP cellular localization predictions and probabilities for T. brucei procyclic form",
      children = pcf_vars
    )
